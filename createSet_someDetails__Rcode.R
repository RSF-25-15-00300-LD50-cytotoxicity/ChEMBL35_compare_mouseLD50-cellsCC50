...
mouse_LD50_cmpnd_id__query <- dbSendQuery(con, 'SELECT cs.molregno, cs.canonical_smiles, act.standard_value, act.standard_units FROM compound_structures cs
...
hcl_tox_cmpnd_id__query <- dbSendQuery(con, 'SELECT cs.molregno, cs.canonical_smiles, act.standard_value, act.standard_units, cp.full_mwt FROM compound_structures cs
													JOIN molecule_hierarchy mh
